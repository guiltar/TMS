...
class TMS_DataWriter_Modification extends XenForo_DataWriter
{
	...
	protected function _getExistingData($data)
	{
		if ($id = $this->_getExistingPrimaryKey($data, 'modification_id')) {
			return array('tms_modification' => $this->_getTmsModModel()->getModificationById($id));
		}
		...
		return array('tms_modification' => $this->_getTmsModModel()->getModificationInStyleByTitle($title, $styleId));
	}
	...
	protected function _preSave()
	{
		...
		if ($this->getOption(self::OPTION_CHECK_DUPLICATE)) {
			if ($this->isInsert() || $this->get('title') != $this->getExisting('title')) {
				$titleConflict = $this->_getTmsModModel()->getModificationInStyleByTitle($this->getNew('title'), $this->get('style_id'));
				if ($titleConflict) {
